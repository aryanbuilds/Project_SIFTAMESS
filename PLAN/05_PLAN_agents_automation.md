...
- **PRIMARY path:** the deterministic real-tool executor (Epic F) — a real, in-process executor over real evidence that SIFTMesh fully controls (the 8 in-process real backends; see PLAN/08_REAL_TOOL_STACK.md). The demo runs on the deterministic real-tool executor. The generic shell adapter, live agents, and A2A remain best-effort fallbacks, never on the demo critical path.
...
**Design (I):** **CAO-as-fallback (explicit):** PRIMARY = the deterministic real-tool executor (real, in-process; see PLAN/08_REAL_TOOL_STACK.md); generic shell, `cao_adapter.py`, and live agents are thin best-effort wrappers; absent CAO/tmux → registry falls back to the deterministic real-tool executor with a logged event. The demo never requires CAO. **Output-schema enforcement is a hard gate:** any adapter result is validated against the required executor schema before collect; failure → `retry_required` (drives the self-correction loop even for live agents). **11-day cut:** I1–I4 MVP; **I5 (CAO) and I6 (live wiring) best-effort/stretch** — the deterministic real-tool executor is the reliable substrate.
...
**Cut rule:** live agents and CAO are the second item in the global cut order (after TUI). Because the deterministic real-tool executor is already the default execution path, cutting them is a **non-loss** for the minimum-winning submission — they only add a "we also drive real agents" bullet to the video.
