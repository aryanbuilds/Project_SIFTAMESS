...
replayable reports. One rule governs all of it: **"LLM proposes, code decides."**
...
`context/agent_capabilities.json`. This follows a "measure capability, label risk" posture: the floor
is the default, Claude is the constrained executor, opencode/codex/gemini are explicit
unconstrained opt-ins, and LiteLLM stays advisory only and never executes a tool.
...
- **Onboarding** (`o` / Agent setup): a `TabbedContent` with **Agents** (a greyed-until-ready
  ...
  key). The cockpit validates provider keys, then saves them to a 600-perm `~/.config/siftmesh/.env`
  (`secrets_env.py` + `tui/auth_actions.py`) and **never** to `siftmesh.toml`.
...
contract), driven by code rather than by the model.
...
- The transition table is a **frozen `dict[State, set[State]]`** in `orchestrator/state_machine.py`.
  The code rejects and logs any LLM-proposed next-state that isn't in the legal set
...
- The engine persists `RunState` atomically (`run_state.json`, temp+rename) for crash-safe `resume`; the
  cockpit's cooperative pause stops at one of these boundaries and stays resumable.
- Modes are config rather than separate code: `manual` (one step per call), `review_only` (plan, stop),
...
**Why boundary ② is strong:** the core real path runs in-process typed Python lib calls. 16 of the
19 tools never spawn a subprocess, so no command string exists to inject into. Only image
extraction (Sleuth Kit), memory triage (Volatility 3), and the Plaso super-timeline shell out, and
those use **fixed-argv, `shell=False`** with no evidence string ever interpolated into a command
(PLAN/08). The reasons this is a
forensic *policy* layer rather than just a harness sandbox, plus how it compares to CAO/Valhuntir, are in
...
[`docs/logs/rocba-memory-RUN-20260612-082630/`](logs/rocba-memory-RUN-20260612-082630/) for raw-log review.
