...
Every finding below is anchored to a real tool call plus the SHA-256 of its source artifact (chain of
...
business and financial documents copied into a **personal** Google Drive. LNK targets under
...
`uploader.db`), which fits a pattern of staging then cleanup.

**Q3. Where was it transferred?**
Two channels. (a) **Personal cloud:** a Chrome download of `installbackupandsync.exe`
(`parse_browser_history`) plus the `C:\Users\fredr\Google Drive\…` LNK targets show data pushed to a
...
`Generic Mass Storage`, `Multiple Card Reader`) plus 20 NTUSER MountPoints2 entries; RecentDocs references
`USB Drive (D:)`.

**Q4. How?**
**Cloud-sync clients plus USB.** ShimCache/Amcache (`parse_amcache_shimcache`) show `GoogleDriveFS.exe`,
...
deletions and the deleted Google-sync DBs.

**Q5. When?**
The USN journal carries per-record FILETIME timestamps across 149,424 creates, 47,966 deletes, and
...
- **Filesystem inventory (`parse_mft_filesystem`, `$MFT`):** 479,359 entries: 371,723 files,
  107,636 directories.
- **Browser history (`parse_browser_history`, 3 databases):** Chrome (`fredr`) 119 visits plus 2 downloads:
...
- **LNK / JumpLists (`parse_lnk_jumplists`, 201 artifacts):** opened-file targets across OneDrive
  ...
  and `Downloads\` (research PDFs plus `SDelete.zip`).
...
- **Removable media (`parse_usb_registry`):** 5 USBSTOR devices (above) plus MountPoints2 GUID volumes.
...
- **Program execution (`parse_amcache_shimcache`):** 517 Amcache plus 391 ShimCache entries; Google Drive,
  Backup-and-Sync, and Teams binaries confirmed run.
- **USN change journal (`parse_usnjrnl`, `$Extend\$UsnJrnl:$J`):** 383,915 records: 149,424 created,
  47,966 deleted, 24,577 renamed (the deleted set includes the Google-sync DBs, indicating exfil cleanup).
...
- **Persistence (`extract_registry_run_keys`, SOFTWARE/user hives):** `GoogleDriveFS`, `GoogleDriveSync`,
  ...
  a GUID-named `C18E42C7…._service_run` that merits manual review.
- **Execution history (`analyze_prefetch`, 200 prefetch files):** AcroRd32, Slack, SmartScreen,
  MicrosoftEdgeUpdate, Adobe ARM, etc., each with run count plus source `.pf`.
- **PowerShell (`parse_evtx_powershell`, host `SRL-FORGE`):** 2× EventID 4104 (script-block) resolved to a
...
egress reached iCloud/Apple (`17.248.x`, `17.57.144.165`), OneDrive/Teams cloud, and `svchost` →
`81.30.144.115:56687` / `213.202.233.104:13939` (flagged for review, not concluded malicious). 16
`malfind` RWX hits sat in system processes (MsMpEng/SearchApp/dllhost/Teams), flagged but not malicious.

## Honest caveats (recorded, never fabricated)

- **`Security.evtx`** failed extraction on a genuine TSK LZNT1 decompression error on this image
  (recorded in `failed[]`).
- **Super-timeline source.** The full-disk Plaso run aborts on this image's **unreadable VSS backup
  NTFS header** (offset ≈ 81 GB, past the 23 GB physical `.E01`, indicating a partial/sparse acquisition). The
...
- **Prompt-injection defense:** evidence strings that looked like instructions were flagged and logged,
  never executed.
...
- Every claim cites `tool_call_id` plus `source_sha256`; unsupported claims are firewalled out of the
  findings (Appendix B of the run's `reports/final_report.md`).
- The committed full ledgers for both runs live under `docs/logs/rocba-disk-RUN-20260612-163324/` and
  `docs/logs/rocba-memory-RUN-20260612-082630/`.
- Reproduce: see [`complete_operation.md`](complete_operation.md). The full ledgers plus replay live in the
