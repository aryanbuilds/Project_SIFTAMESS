...
This hashes + seals → plans → extracts (Sleuth Kit) → re-ingests the carved artifacts → parses
(evtx/registry/prefetch/MFT/USN/Plaso) → critiques → reports. Full staged + memory + portions flows:
[`../RUNBOOK.md`](../RUNBOOK.md). The committed real-evidence record (run dirs stay off-repo per
CLAUDE §2B): [`findings_rocba.md`](findings_rocba.md), [`accuracy_report.md`](accuracy_report.md),
[`execution_logs_sample.md`](execution_logs_sample.md), [`complete_operation.md`](complete_operation.md).
...
`siftmesh tui` (no arg) opens the minimal home. Press **`o`** for onboarding (an **Agents** tab and a
**Tier-2 judge** tab — LiteLLM keys are validated and saved to a 600-perm `~/.config/siftmesh/.env`,
never to the committed config).
...
- **Tiers, not theatre.** `siftmesh agents list` labels each agent T0–T3. Only Claude reaches the typed
  tools through the strict-MCP boundary (T1); opencode/gemini/codex are honestly labelled unconstrained
  opt-ins (T2). LiteLLM is advisory-only (T3, tool-less judge). Tiers never gate dispatch — they make
  the containment posture visible.
- **Real-only.** No mock tool backends; a missing backend fails closed, never fakes output. The results
  in `docs/` are from the real ROCBA disk + memory runs, not a demo fixture.
- **Run dirs are off-repo** (CLAUDE §2B): the `docs/` set is the curated, verifiable record; the
  maintainer can show the complete ledgers live on the workstation.
