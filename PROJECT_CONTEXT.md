...
**Agent neutrality + onboarding (Epic Q, round 1; PLAN/13).** SIFTMesh is not Claude-only. A single config-driven `HeadlessAdapter` (`adapters/headless.py`) launches **any** CLI agent from its `agent_profiles.yaml` `launch_argv` recipe — `gemini_headless`/`codex_headless`/`openclaw_headless` ship alongside claude/opencode, so adding an agent is a **profile row, not a new adapter**. `--agent gemini|codex|opencode|openclaw|claude|deterministic` reorders the same fallback chain (zero FSM change). **Onboarding:** `siftmesh doctor --agents` / `siftmesh agents list` / `agents inspect <id>` run `probe_agents()` → a typed `AgentCapabilityMap` (`schemas/agent_capabilities.py`, optionally written to `context/agent_capabilities.json`) reporting per-agent present/authed/tool-reachable + the chosen default. **Honest tool-reachability:** only Claude (`mcp_strategy=claude_flag`) reaches the typed tools today; gemini/codex/openclaw are `verify-live` (run, but no typed tools until per-run MCP wiring lands in round 2 / ACP) and are never the silent default. Decision is **headless-first, ACP second** (ADR rationale in `PLAN/13`): the ACP Python SDK is real+viable, but some agents silently ignore MCP-over-ACP, so the headless `--mcp-config` path ships first; round 2 adds the ACP client + `session/request_permission` permission gate.
