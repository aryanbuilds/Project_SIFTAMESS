...
| Δ5 | **Linux-first (dev + target = SANS SIFT / Ubuntu)** — `pathlib` discipline (good hygiene), **Ubuntu-primary CI (= the SIFT target)**, and a **deterministic, zero-live-LLM REAL-tool executor spine** (real in-process tools: `evtx`/`regipy`/`pyscca`/`mft` over real maintainer-provided evidence — *not* a mock; see `PLAN/08_REAL_TOOL_STACK.md`). | Development and target are both Linux (SANS SIFT / Ubuntu); the plan is authored on Windows but no code is built or run there, so Windows is not a constraint (see `PLAN/08 §0.1`). The in-process real tools install natively and run locally now with no API keys/network, while the gated Plaso/SIFT-lane backends are upgrades on top. |
...
| Δ7 | **CAO / live agents demoted to a best-effort upgrade lane**, never on the demo critical path. | CAO is moderate-maturity and env-dependent; depending on it risks the whole demo. The deterministic REAL-tool executor (real in-process `evtx`/`regipy`/`pyscca`/`mft` over real evidence; see `PLAN/08_REAL_TOOL_STACK.md`) is the reliable substrate; live agents/CAO remain a best-effort upgrade lane. |
...
| 9 | Jun 12 | **Spine lock:** full deterministic real-tool run + self-correction; golden run; CI | M3 golden, M5 e2e, M6 CI; J6/J8 wiring | **cut live agents/CAO** |
...
**Cut order (first→last):** O (TUI) → **A2A / Epic P** → live agents/CAO (the deterministic real-tool executor is default) → `replay.html` (keep text replay) → MCP tools beyond the 4 core → optional prompt-injection demo event. *(Build priority is the inverse: P1 core > P2 CAO > P3 A2A > P4 TUI — least-important cut first.)*
**Never cut:** the 8 artifacts · the self-correction sequence · the bypass suite.

---

## 9. Minimum winning submission (definition of done)

A **zero-live-LLM, deterministic REAL-tool run** of
`siftmesh run ./examples/demo_case --evidence ./examples/demo_case/evidence --auto`
(and the `--auto-human-loop` variant for the video), using the **real in-process tools** (`evtx`/`regipy`/`pyscca`/`mft`; see `PLAN/08_REAL_TOOL_STACK.md`) over the **real demo evidence**, that, on a fresh clone on Ubuntu/SANS SIFT with **no API keys and no network**:
...
2. Shows a visible, **genuine** self-correction sequence — an under-specified first-pass contract yields a real claim that the critic rejects → ultraworker-retry under a tightened contract → corrected evidence-backed claim — with the unsupported claim appearing only in the rejected section (never as a fact).
...
|---|---|---|---|---|---|
| R1 | Self-correction flaky in demo | Med | High (tiebreaker) | Deterministic real-tool executor over fixed committed real evidence, no LLM/randomness on the guaranteed path (see `PLAN/08_REAL_TOOL_STACK.md §6`); M5 asserts ≥10/10 | K3, M5 |
...
| R5 | Live-agent/CAO integration eats critical path | High | Med | The deterministic real-tool executor is default; CAO is upgrade lane, hard-cut Day 9 | I, roadmap |
