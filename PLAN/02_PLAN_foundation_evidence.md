...
|---|---|---|---|---|---|---|---|
| A1 | Packaging & license | `pyproject.toml` (hatchling/setuptools), Apache-2.0 `LICENSE`, metadata, `console_scripts` entry `siftmesh = siftmesh_core.cli:app`, pinned deps (typer, pydantic≥2, jinja2, structlog, pyyaml, mcp, pytest, regipy). | `pyproject.toml`, `LICENSE`, `siftmesh_core/__init__.py` | — | `pip install -e .` succeeds; `siftmesh` on PATH; LICENSE is Apache-2.0. | S | dep drift → pin minimums |
...
| A7 | CI pipeline | GitHub Actions on `ubuntu-latest` (= the SANS SIFT / Ubuntu target; a Windows runner is optional and not required), Python 3.11, install + `pytest` + ruff + mypy. | `.github/workflows/ci.yml` | A6 | CI green on Ubuntu. | S | path separators → pathlib discipline (kept as hygiene) |
