...
> [`logs/README.md`](logs/README.md)). This file walks through one trace. Only the heavy
> *derived* trees (per-tool `results/`, the carved `evidence/extracted/`, the 11 GB Plaso
> `super_timeline/`) and the raw evidence images stay off-repo, for size and chain of custody.
...
Below is the complete chain, each link a real ledger line.
...
**(2) The tool execution it points to**, from `audit/tool_calls.jsonl`, `TOOL-004`. The
`source_sha256` matches (custody match), and the record carries start/end timestamps, status,
backend, and the structured-output path:
...
**(3) The dispatch that ran it**, from `audit/agent_calls.jsonl`, `AGENT-004`. Each dispatch is one
line, recording the profile and adapter, the attempt number, and start/end times:
...
**(4) The orchestration events**, from `audit/orchestration_events.jsonl`. Every state transition and
dispatch is a timestamped event, and transitions carry `duration_ms`:
...
image's SHA-256. `f2eb856d…` is the sealed manifest hash, so this is chain of custody back to the raw
evidence:
...
fully timestamped, anchored to the raw image's hash, with both structured and raw output preserved:
...
`claims/injection_alerts.jsonl`. They are logged only, never executed. This run flagged 3,949 such
strings across the 5.7 M-event timeline, 383 K USN records, and 479 K-file `$MFT`:
...
evtx, regipy, and Plaso, with no LLM in the executor. There are no executor token counts to report.
`audit/token_budget.jsonl` records the profile-routing decisions the engine made (the escalation
