...
| 2 | **IR accuracy / no hallucination** | `"$RUN/claims/claim_ledger.jsonl"` — every claim cites `tool_call_id`+`source_sha256`; unanchored claims are confined to Appendix B of `reports/final_report.md`; `docs/accuracy_report.md` |
...
| 5 | **Audit trail quality** | `"$RUN/audit/*.jsonl"` + `uv run siftmesh replay "$RUN"`; annotated in `docs/execution_logs_sample.md`; chain of custody in `docs/evidence_integrity.md` |
