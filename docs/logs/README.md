...
evidence on the SANS SIFT workstation. We committed them so a judge can inspect any finding and trace it
back to the exact tool execution without the workstation. See a guided walkthrough of one trace in
...
To keep the repo lean, three large *derived* trees stay on the workstation. They are outputs the ledgers
...
We never commit the raw evidence images themselves (chain of custody; they stay read-only on
the box). `injection_alerts.jsonl` contains *truncated* evidence-derived string fragments. We
**logged** them and never executed them.
