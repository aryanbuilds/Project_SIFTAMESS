...
final report asserts as fact. Deterministic SIFTMesh code makes every security-relevant
decision, and it runs regardless of which agent or harness drives it: Claude
Code, OpenCode, a generic shell agent, the deterministic floor, or (later) a remote A2A
agent. The guardrails do not live in the prompt. They live in the gate that the prompt's output
must pass through.
...
The outer ring, the policy layer, is the differentiator. It holds even if the inner
...
**Framework framing.** This model is analysed in three layers, per current (2025 to 2026)
practice. STRIDE / data-flow covers the deterministic SIFTMesh *system* (vault, path
policy, run-dir, audit). The **OWASP Top-10 for LLM Applications 2025** covers the *model*
layer (the T1 to T9 table in §3). An **agentic overlay** covers the *autonomy* layer (§3A):
the **OWASP Top-10 for Agentic Applications (ASI01-ASI10, 2025-12-09)**, **OWASP Agentic
Threats & Mitigations**, and the **MAESTRO** seven-layer method, with **MITRE ATLAS**
technique IDs naming the attacker move. Governance references: **NIST AI RMF 1.0**,
the **Generative-AI Profile (NIST AI 600-1)**, and **SSDF SP 800-218** (note: SP 800-218A
was withdrawn after EO 14110's rescission, 2025-01-20, and is cited only as historical).
In the literature's terms, SIFTMesh's anti-injection posture is **Plan-Then-Execute +
Action-Selector** (Beurer-Kellner et al., arXiv:2506.08837) with CaMeL-style provenance
tracking (arXiv:2503.18813), realised by the critic's claim-anchoring rather than a prompt that
...
Each row is a quintuple: threat, OWASP LLM Top 10 (2025) mapping, code-enforced control,
owning module, bypass test, and residual risk. The bypass tests live in
...
## 3A. Agentic-threat overlay (OWASP Top-10 for Agentic Applications, ASI01 to ASI10)

The §3 table maps the *model* layer (OWASP LLM Top-10). SIFTMesh is an **autonomous
agentic** controller, so it must also answer the **agentic** standard. _ID note: OWASP's "Agentic
...
*scoped-out* (not reachable in this build, with the reason stated), or *open* (deferred, tracked).
...
| ASI05 Unexpected Code Execution | L4 | - | **covered** | no shell/eval/dynamic dispatch; 16/19 tools in-process, 3 fixed-argv `shell=False` (T5) |
...
These boundaries are drawn in [`architecture.md`](architecture.md) and the
[security-boundaries diagram](diagrams/security_boundaries.mmd). Crossing any of them runs a
code-decided gate:

1. **Evidence-vault boundary.** Originals are read-only, integrity hashed before
   analysis, and nothing writes back (T4). → `evidence/readonly.py`, `evidence/manifest.py`,
...
5. **Critic boundary.** No claim becomes a fact without a matching tool-call anchor, and the
...
- Resolve `(run_root / rel)` with `Path.resolve()`. This **follows symlinks**, so a
  symlink placed inside the run dir that points outside resolves to its real target
...
- Unresolvable paths (symlink loops, etc.) raise `PathPolicyViolation` and never silently
  pass.
...
A path that escapes **every** trusted root raises `ValueError`. Admitting the run
...
- The allowlist (`ALLOWED_TOOLS`) and the forbidden set (`FORBIDDEN_TOOLS` = the 7
  CLAUDE §6 names) are explicit `frozenset`s. There is **no dynamic dispatch, no `eval`,** so a
...
- `assert_tool_allowed(name)` runs at **registration time**: a forbidden name raises
  ...
  `ToolNotAllowedError("not in the forensic allowlist")`. No code path
...
- **Never pass raw evidence bytes to a prompt.** The prompt builder emits only structured
  `{path, sha256}` rows. A hostile filename or value cannot smuggle a payload through the
...
defenses are *architectural*, not the scanner: (1) the agent has **no network/exfil tool**, since the
...
`{path, sha256}` rows), giving structural data/instruction separation; (3) the **critic** gates every
...
models, so it is kept opt-in). The honest claim is **containment + traceability, not prevention**: a
...
- **Valhuntir** (per the plan's competitive analysis) adds an HMAC-signed approval ledger.
  It is strong on tamper-evidence, but it is still an approval/audit layer, not a forensic
...
matter how permissive its profile is**. No `--yolo` turns off
`safe_write_path`, the tool allowlist, or the critic. SIFTMesh treats *evidence as
hostile in code* (spotlighting + an injection ledger + a critic consequence) rather than as a
...
- **Critic is structural, not semantic.** It proves a claim is *anchored* to a real tool
  call; it cannot prove the claim is *true*. A well-anchored-but-wrong claim can pass, and
...
- **Path policy is canonicalize-then-check (TOCTOU).** `safe_write_path` resolves on a filesystem
  ...
  now fail closed (fixed in Epic L); the gate assumes `rel` is already decoded, so callers must never
...
- **We do not cryptographically sign tools/artifacts.** That is industry parity for this class (Velociraptor
...
assertion, not a finding"). The committed full ledgers backing these results are at
[`docs/logs/rocba-disk-RUN-20260612-163324/`](logs/rocba-disk-RUN-20260612-163324/) and
[`docs/logs/rocba-memory-RUN-20260612-082630/`](logs/rocba-memory-RUN-20260612-082630/).
