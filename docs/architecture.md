...
**Why boundary ② is strong:** the core real path is **in-process typed Python lib calls** — 16 of the
19 tools never spawn a subprocess, so there is no command string to inject into at all. Only image
